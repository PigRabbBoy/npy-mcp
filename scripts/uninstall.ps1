...
function Remove-Json($file, $key) {
  ...
  $servers = $cfg[$key]
  if (-not $servers -or -not ($servers -is [hashtable]) -or -not $servers.ContainsKey("notion-py")) { return "ABSENT" }
  $servers.Remove("notion-py")
...
function Remove-Toml($file) {
  ...
  if ($content -notmatch '\[mcp_servers\.notion-py\]') { return "ABSENT" }
  Copy-Item $file "$file.bak-$(Get-Date -Format yyyyMMddHHmmss)"
