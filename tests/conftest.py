"""Pytest config — ensure npy-core src on path for all tests."""

import os
import sys

# Ensure npy-core is importable
_CORE_SRC = os.path.join(os.path.dirname(__file__), "..", "packages", "npy-core", "src")
_CORE_SRC = os.path.abspath(_CORE_SRC)
if _CORE_SRC not in sys.path:
    sys.path.insert(0, _CORE_SRC)

# Ensure npy-cli is importable
_CLI_SRC = os.path.join(os.path.dirname(__file__), "..", "packages", "npy-cli", "src")
